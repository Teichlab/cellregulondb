...
def fetch_symbols():
    ...
    all_symbols = cursor.execute("SELECT id, name, properties->>'is_TF' FROM nodes").fetchall()
    match_results = []
        ...
        if response.ok:
            ...
            for symbol in batch_symbols:
                if symbol[1] in data.keys():
                    if UPDATE_DATABASE:
                        node = cursor.execute("SELECT id, properties FROM nodes WHERE id = ?", [symbol[0]]).fetchone()
                    ...
                    good += 1
                    match_results.append({"gene_in_db":symbol[1], 'is_TF':symbol[2], **data[symbol[1]]})
                else:
                    bad += 1
